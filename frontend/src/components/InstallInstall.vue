<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            Install # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            Install
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <String label="DeliveryId" v-model="value.deliveryId" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/installs'), this.value)
                        } else {
